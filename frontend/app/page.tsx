...
export default function Home() {
  ...
  // Helper to fetch and sync fresh logs
  ...
  // Initial Bootup Hook: Loads data safely on client mount to prevent Hydration Mismatch
  useEffect(() => {
    ...
    fetch(`${API_BASE}/targets/${user.user_id}`)
      ...
      .catch((err) => console.error("Error loading profile targets:", err));

    refreshLogs(user.user_id);
  }, []);

  // Compute and group history logs by date dynamically whenever foodLogs updates
  useEffect(() => {
    const grouped: Record<string, number> = {};

    foodLogs.forEach((food) => {
      const logDate = food.date || new Date().toISOString().split('T')[0];
      if (!grouped[logDate]) {
        grouped[logDate] = 0;
      }
      grouped[logDate] += Number(food.calories || 0) * Number(food.quantity || 1);
    });

    const historyData = Object.entries(grouped)
      .map(([date, calories]) => ({
        date,
        calories,
      }))
  ...
  // Sync tasks to localStorage *only* after initial hydration load has completed
  useEffect(() => {
    if (isHydrated) {
  ...
  const updateQuantity = async (logItem: FoodLogItem, newQuantity: number) => {
    if (newQuantity < 1) {
      deleteFood(logItem.id);
      return;
    }
    ...
    try {
      await fetch(`${API_BASE}/food-log/${logItem.id}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      console.error("Failed to update log item quantity:", err);
    }
  };
  ...
  // Derived tracking values (Correctly factoring in unit quantities)
  ...
  (sum, food) => sum + Number(food.fat || 0),
  0
);
  ...
  const bestDay =
    history.length > 0
      ? history.reduce((best, current) => (current.calories > best.calories ? current : best))
      : null;
  ...
  const macroCompletionPercentage = targets.calories > 0 
    ? Math.min((consumedCalories / targets.calories) * 100, 100) 
    : 0;
  ...
  const goalDescriptions: Record<string, string> = {
    ...
    "🔥 Lose Weight": "Calorie deficit while preserving muscle mass through adequate protein intake.",
  };
    ...
    <main className="shell">
      ...
      <div className="main-content">
        ...
        <div className="top-row">
          <div>
            ...
            <p className="greeting-sub">
              You&apos;re {Math.max(0, targets.calories - Math.round(consumedCalories))} kcal away from your goal today
            </p>
          </div>
        ...
        <div className="search-wrap">
          ...
          >
            <button className="qty-adjust-btn" onClick={() => setQuantity(Math.max(1, quantity - 1))}>
              -
            </button>
            <span style={{ fontSize: "13px", color: "#aaa" }}>Quantity: {quantity}</span>
            <button className="qty-adjust-btn" onClick={() => setQuantity(quantity + 1)}>
              +
            </button>
          </div>
        ...
        {/* NUTRITIONAL KPI SUMMARY STRIP */}
        <div className="stats-row">
          <div className="stat-card">
            <div className="stat-icon">🔥</div>
            <span className="stat-value">{Math.round(consumedCalories)}</span>
            <span className="stat-unit">/ {targets.calories} kcal</span>
            <div className="stat-label">Calories</div>
            <div className="stat-bar">
              <div
                className="stat-fill"
                style={{
                  width: `${macroCompletionPercentage}%`,
          ...
          <div className="stat-card">
            ...
            <div className="stat-bar">
              <div
                className="stat-fill"
                style={{
                  width: `${targets.protein > 0 ? Math.min((consumedProtein / targets.protein) * 100, 100) : 0}%`,
          ...
          <div className="stat-card">
            ...
            <div className="stat-bar">
              <div className="stat-fill" style={{ width: "70%", background: "linear-gradient(90deg,#38bdf8,#0ea5e9)" }} />
            </div>
          </div>
          ...
          <div className="stat-card">
            ...
            <div className="stat-bar">
              <div className="stat-fill" style={{ width: "82%", background: "linear-gradient(90deg,#34d399,#10b981)" }} />
            </div>
          </div>
        </div>

        {/* MID SEGMENT MACRO GRAPHICAL TRACKER */}
        <div className="mid-row">
          <div className="macros-card">
            <div className="ring-wrap">
              <svg className="ring" width="120" height="120" viewBox="0 0 120 120">
                ...
                <circle
                  ...
                  strokeLinecap="round"
                  strokeDasharray="314"
                  strokeDashoffset={314 - (314 * macroCompletionPercentage) / 100}
                />
                <defs>
              ...
              <div className="ring-center">
                <div className="ring-cal">{Math.round(macroCompletionPercentage)}%</div>
            ...
            <div className="macro-list">
              ...
              {[
                {
                  name: "Carbs",
                  val: `${Math.round(consumedCarbs)}g`,
                  pct: `${carbTarget > 0 ? Math.min((consumedCarbs / carbTarget) * 100, 100) : 0}%`,
                  color: "#f97316",
                },
                {
                  name: "Protein",
                  val: `${Math.round(consumedProtein)}g`,
                  pct: `${targets.protein > 0 ? Math.min((consumedProtein / targets.protein) * 100, 100) : 0}%`,
                  color: "#8b5cf6",
                },
                {
                  name: "Fat",
                  val: `${Math.round(consumedFat)}g`,
                  pct: `${fatTarget > 0 ? Math.min((consumedFat / fatTarget) * 100, 100) : 0}%`,
                  color: "#34d399",
                },
              ].map((m) => (
          ...
          <div className="goal-card">
            <div>
              ...
              <div className="goal-desc">{goalDescriptions[userGoal]}</div>
            </div>
            <div className="goal-badge">
              ...
              {userGoal === "🔥 Lose Weight" && "🔥 Fat Loss"}
            </div>
          </div>
        </div>

        {/* BOTTOM METRICS CARD ROW */}
        <div className="bottom-row">
          ...
          <div className="bottom-card">
            ...
            {foodLogs.length === 0 ? (
              <div style={{ color: "#666", fontSize: "13px", padding: "10px 0" }}>No foods logged today</div>
            ) : (
              foodLogs.map((log) => (
                  ...
                  <div style={{ display: "flex", gap: "10px", alignItems: "center", minWidth: 0, flex: 1 }}>
                    ...
                    <div style={{ minWidth: 0 }}>
                      ...
                      <div style={{ fontSize: "11px", color: "#666" }}>
                        {Math.round((log.calories || 0) * (log.quantity || 1))} kcal
                      </div>
                    </div>
                  </div>

                  <div style={{ display: "flex", alignItems: "center", gap: "6px", flexShrink: 0 }}>
                    <button className="log-qty-btn" onClick={() => updateQuantity(log, Number(log.quantity || 1) - 1)}>
                      -
                    </button>
                    <span style={{ fontSize: "12px", color: "#aaa", minWidth: "16px", textAlign: "center" }}>
                      {log.quantity}
                    </span>
                    <button className="log-qty-btn" onClick={() => updateQuantity(log, Number(log.quantity || 1) + 1)}>
                      +
                    </button>
                    <button className="log-delete-btn" onClick={() => deleteFood(log.id)}>
                      ✕
                    </button>
                  </div>
                </div>
            ...
            {/* Activity subsection brought clean into the column */}
            <div style={{ marginTop: "24px" }}>
      ...
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .search-icon {
          position: absolute; left: 18px; top: 27px; transform: translateY(-50%);
        ...
        .ring { transform: rotate(-90deg); transition: stroke-dashoffset 0.35s ease; }
        .ring-center {
